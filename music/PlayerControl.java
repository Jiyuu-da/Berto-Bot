...
public class PlayerControl extends ListenerAdapter
{
    ...
    // New method for handling the .search command
    private void searchAndPlay(GuildMusicManager mng, final MessageChannel channel, String query) {
        playerManager.loadItemOrdered(mng, "ytsearch:" + query, new AudioLoadResultHandler() {
            @Override
            ...
            public void noMatches() {
                channel.sendMessage("No results found for: " + query).queue();
            }

            @Override
            public void loadFailed(FriendlyException exception) {
                channel.sendMessage("Could not load search results for: " + query).queue();
            }
        });
    }
    ...
    public void onMessageReceived(MessageReceivedEvent event)
    {
        ...
        else if (".play".equals(command[0]))
        {
            ...
            {
                if (command.length == 2) {
                    // If the command has 2 parts, it's likely a URL
                    loadAndPlay(mng, event.getChannel(), command[1], false);
                }  else if (command.length > 1) //Commands has 2 parts, .play and url or song name.
                {
                    if (command[1].startsWith("http")) {
                        // If the command has 2 parts and starts with "http", it's likely a URL
                        loadAndPlay(mng, event.getChannel(), command[1], false);
                    } else {
                        // If the command has 2 parts and doesn't start with "http", it's a song name
                        searchAndPlay(mng, event.getChannel(), command[1]);
                    }
                }
            }
        }
